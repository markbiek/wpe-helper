    ...
    public function handle() {
        ...
        while (true) {
            try {
                ...
                if ($tries > $max) {
                    //TODO - exception
                    Log::error("{$install->name} exceeded number of clone retries");
                    break;
                }
                ...
                if (stripos($e->getMessage(), 'not found')) {
                    ...
                    $domain = preg_replace("/^(.*?)\./", "", $install->primary_domain);
                    $repoUrl = sprintf($baseUrl, $domain);
                } else {
                    //TODO - maybe notification?
                    Log::error("Error cloning {$install->name}: " . $e->getMessage());
                }
            }
        }
        ...
        $process = Proc::exec('wp plugin list --update=available --fields=name --format=json', $wcPath, $output);
        ...
        foreach ($plugins as $plugin) {
            ...
            if ($skip) {
                Log::info("{$install->name}: Skipping {$plugin->name} because it was blacklisted");
                continue;
            }

            try {
                Log::debug("{$install->name}: Updating {$plugin->name}");
                $process = Proc::exec("wp plugin update {$plugin->name}", $wcPath, $output);
        ...
        try {
            //Push to WPE
            $wc->remote('add', 'production', "git@git.wpengine.com:production/{$install->name}.git");
            Proc::exec([
                'git',
                'clean',
                '-d',
